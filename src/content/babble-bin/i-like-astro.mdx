...
often when the topic of astro comes up, all i can really do i mention:
...
- a [webpack](https://webpack.js.org/) site, not making very much use of webpack
...
previous to using astro, my intent was to keep things as similar as possible to the 'old web' / geocities vibes

the most important thing was making sure the look and feel felt correct, but i also thought it'd be interesting
to avoid using any sort of build tool, and just hand-rolling the html/css/js

this was fine when the site was new, but i sort of kept layering on features and buttons and subpages, therefore
...
picking astro did feel like i was going back on one of my goals of using no build system, but after i was done i
was able to focus a lot more on the content and feel than the structure, so the version that stands today is
thankfully very much an improved version of what stood before it, let's dig into it a bit
...
this is the homepage, where most of the content and effort has gone into this site since i started adding to it in 2019

each div in this screen has its own thing going on, and that is _very_ much intentional
...
most of the pages on this site are a bit too busy to contain within one code block, but if you check out the source for
...
even though there is a lot of css and code required for composition, it would be much more complex if i was still limiting
myself to just plain html

(and no, i know you could probably go crazy with html `<template>` and some custom javascript, but that did not feel like
...
the content collections can feed in a [directory of files, such as `.mdx` files](https://github.com/jackharrhy/www.cs.mun.ca-jaharrhy/tree/02b4a8bd18d5b0c585cf37648bb424a465f9dc18/src/content/babble-bin),
...
alongside methods to render that content easily into and astro page
...
[`@astrojs/rss`](https://docs.astro.build/en/guides/rss/), is used to power the babble bin's [rss feed](https://www.cs.mun.ca/~jaharrhy/babble-bin/rss.xml), this i can include in this blog post!
...
the structure of my mun site is great, as in its almost complete lack of structure in some ways, but also a structure
that is welcoming to any content that wishes to call it home
...
you'll find pages like these strewn across my mun site, [with nice and tidy `.astro` files implementing them](https://github.com/jackharrhy/www.cs.mun.ca-jaharrhy/blob/36920e6bf4b7063fddb8788b991512ae6cc1132b/src/pages/gamedev/jams/growthjam-feb2023/index.astro)
...
and lastly, one of my only uses of an interesting feature of astro: [a page] written using [SolidJS](https://www.solidjs.com/)
...
without a virtual dom that feels like react from a syntax perspective, it made sense to reach for it, since astro made integrating it trivial

there's no actual content here though, i got the basemap working in full screen, and _shipped it_ ⛴️
